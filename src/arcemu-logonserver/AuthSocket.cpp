...
void AuthSocket::HandleProof()
{
	...
	//printf("[LOGON] - Successfully authenticated\n");
...
{
	//printf("[LOGON] - SendChallengeError\n");
	uint8 buffer[3];
	buffer[0] = buffer[1] = 0;
	buffer[2] = Error;
...
{
	//printf("[LOGON] - HandleRealmList\n");
	sInfoCore.SendRealms(this);
}

void AuthSocket::HandleReconnectChallenge()
{
	//printf("[LOGON] - HandleReconnectChallenge\n");
	// No header
	if(readBuffer.GetContiguiousBytes() < 4)
		return;
...
{
	//printf("[LOGON] - HandleReconnectProof\n");
	/*
